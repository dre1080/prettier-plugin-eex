...
const embedEx = (path, _print, textToDoc, options) => {
  const text = options.originalText;
  if (!text.match('~L"""')) {
    return options.originalText;
  }

  const newText = text.replace(/([\s]*)~L"""([\s\S]*?)\S*?"""/gm, (match, spaces, html) => {
    // const matcher = match.match(/<%(#=|%=|=|%|#)?([\S\s]*?)\s*%>/m);
    // const tag = matcher[1] || '';
    // const expression = matcher[2];
    console.log(html);
    const onlySpaces = spaces.replace(/(\r\n|\n|\r)/gm, '');
    const newDoc = prettier.format(html, { ...options, parser: 'eex' });
    const newDocIndented = newDoc.trim().replace(/(\r\n|\n|\r)+/gm, `$&${onlySpaces}`);
    return `${spaces}~L"""\n${onlySpaces}${newDocIndented}\n${onlySpaces}"""`;
  });
...
module.exports = {
  ...
  'ex-ast': {
    embed: (path, _print, textToDoc, options) => {
      try {
        return embedEx(path, _print, textToDoc, options);
      } catch (e) {
        console.error(e);
        return options.originalText;
      }
